...
import static com.moandjiezana.toml.ResourceLocationConverter.IDENTIFIER_CONVERTER;
  ...
  static Results run(String tomlString) {
    ...
    for (int i = index.get(); i < tomlString.length(); i = index.incrementAndGet()) {
      ...
      if (c == '#' && !inComment) {
        ...
        ResourceLocation id = IDENTIFIER_CONVERTER.convert(tomlString, index, new Context(null, line, results.errors));
        
        if (id != ResourceLocation.INVALID) {
          if (id.isKey()) {
            identifier = id;
          } else if (id.isTable()) {
        ...
        results.errors.invalidTextAfterResourceLocation(identifier, c, line.get());
      }
    }
